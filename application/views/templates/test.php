...
<!-- Scroll to Top Button-->
   ...
   <script src="<?= base_url('assets/'); ?>leaflet/leaflet-fullscreen-master/Control.FullScreen.js"></script>
    ...
    <script>
        ...
        var map = L.map('mapupt-index').setView([-7.6709737, 112.7288216], 7.3);
        ...
        L.tileLayer('https://tile.openstreetmap.org/{z}/{x}/{y}.png', {
         ...
         //  kabupaten/kota di jawa timur polygon geo json
        ...
        // icon untuk kantor UPT
        var iconUPT = L.icon({
        iconUrl : '<?= base_url('assets/'); ?>img/marker/city.png',
        iconSize : [30,25],
        });

        // MARKER dengan cluster Kantor UPT
        ...
        <?php foreach ($data_sebaran_upt  as $key => $value) { ?> 
            var sebaranUPT = L.marker([<?= $value->lat_upt ?>, <?= $value->long_upt ?>], { icon:icon} )
            .bindPopup(
            '<div class="container px-1 py-1">'+
              '<h5 style="text-overflow: ellipsis; white-space: nowrap; overflow: hidden;"><?= $value->nama_upt ?></h5> <span class="badge badge-pill badge-danger" style="font-size:12px;"><i>telah ter-PHK</i></span> '+
              ...
              '<i class="fa-solid fa-building" style="margin-bottom: 10px;margin-right: 10px;"></i>dari , <?= $value->ket_upt ?><br />'+
              '<i class="fa-solid fa-square-phone" style="margin-bottom: 10px; margin-right: 10px;"></i>No. Handphone , <?= $value->lat_upt ?><br />'+
              '<i class="fa-solid fa-hospital-user" style="margin-bottom: 10px;margin-right: 10px;"></i>Berkebutuhan Khusus , <?= $value->long_upt ?> </p></div>');
        ...
        var overlays = {"Kantor UPT": markersUPT};
